...
/**
 * Admin pengaduan list page.
 * Features: search, status filter, pagination, link to detail.
 */
export default function PengaduanListPage() {
  ...
  const { data, isLoading, refetch, isFetching } =
    trpc.admin.listPengaduan.useQuery(
      {
        page,
        pageSize,
    ...
    <div className='space-y-6'>
      <div>
        ...
        <p className='text-sm text-muted-foreground'>
          Kelola seluruh pengaduan dari masyarakat.
        </p>
      </div>

      <Card>
        <CardHeader className='flex flex-row items-center justify-between gap-4 pb-4'>
          <CardTitle className='text-base'>Daftar Pengaduan</CardTitle>
          <Button
            variant='ghost'
            size='icon'
        ...
        <CardContent className='space-y-4'>
          <div className='flex flex-col gap-3 sm:flex-row'>
            <div className='relative flex-1'>
              ...
              <Input
                placeholder='Cari nomor tiket atau nama pelapor...'
                value={search}
                  ...
                  <tbody className='divide-y divide-border'>
                    {rows.map((row) => (
                      ...
                      >
                        ...
                        <td className='py-3'>{String(row.nama)}</td>
                        <td className='py-3'>
                          <Badge variant='outline'>
                            {formatKategori(String(row.kategori))}
                          </Badge>
                        </td>
                        <td className='py-3'>
                        ...
                        <td className='py-3'>
                          <Button variant='ghost' size='icon' asChild>
                            <Link
                              href={`/admin/pengaduan/${String(row.id)}`}
                              title='Lihat detail'
                            >
              ...
              {pagination && (
                <div className='flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between'>
                  <div className='flex items-center gap-2 text-xs text-muted-foreground'>
                    ...
                    <span>dari {pagination.total} pengaduan</span>
                  </div>
