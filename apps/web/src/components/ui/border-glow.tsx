...
function parseHSL(hslStr: string): HSLColor {
  ...
  return { h: parseFloat(match[1] ?? '40'), s: parseFloat(match[2] ?? '80'), l: parseFloat(match[3] ?? '80') }
}
...
function buildGradientVars(colors: string[]): Record<string, string> {
  const vars: Record<string, string> = {}
  for (let i = 0; i < 7; i++) {
    const c = colors[Math.min(COLOR_MAP[i] ?? 0, colors.length - 1)] ?? colors[0] ?? '#fff'
    const key = GRADIENT_KEYS[i] ?? `--gradient-${i}`
    vars[key] = `radial-gradient(at ${GRADIENT_POSITIONS[i]}, ${c} 0px, transparent 50%)`
  }
  vars['--gradient-base'] = `linear-gradient(${colors[0] ?? '#fff'} 0 100%)`
  return vars
}
